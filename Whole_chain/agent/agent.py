...
class FunctionRegistry:
    ...
    def _extract_function_metadata(self, func: Callable) -> Dict[str, Any]:
        ...
        doc = inspect.getdoc(func) or "Nothing Available"
...
class Agent:
    def __init__(self, model: str = "llama-8b-8192"):
...
Response (SINGLE LINE JSON):"""  
    ...
    def process_query(self, query: str)-> Any:
        if not self.registry.functions:
            raise ValueError("There is no function is registerd with the agent.")
        ...
        try:
            ...
            if func_name not in self.registry.functions:
                raise ValueError(f"Function {func_name} is not found here . Available functions: {','.join(self.registry.list_functions())}")
