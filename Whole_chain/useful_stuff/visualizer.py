...
Remember: Return ONLY the JSON object, no other text."""
            try:
                ...
                print(f"Failed to parse LLM response into Json form : {response}")
                return{
                    "result": None,
...
class BarGraph(BaseChart):
    def plot(self) -> Optional[plt.figure]:
        ...
        self.ax.set_xlabels(self.data['x-axis'])
        self.ax.set_ylabels(self.data['y-axis'])
        self.ax.set_title(self.data['title'])
        plt.xticks(rotation=45)
        plt.tight_layout()
        return self.fig
...
class ScatterChart(BaseChart):
    def plot(self) -> Optional[plt.Figure]:
        if not self._vaidate_data():
            return None
        categories = list(self.data['dataitems'].keys())
        values = list(self.data['datapoints'].values())
